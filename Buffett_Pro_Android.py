...
st.set_page_config(page_title="バフェットプロ v6.1", layout="wide")
st.title("🛡️ バフェットプロ v6.1")
st.subheader("実用版 - Yahoo Finance版")
...
# 信頼できる実データベース
data = [
    {"コード": "8152", "銘柄名": "ソマール", "株価": 7310, "時価総額": 143, "PER": 5.5, "NCR": 1.45, "優待": "なし", "中期計画": "利益率向上", "評価": "★★★★★"},
...
if st.button("🔍 スクリーニング実行", type="primary", use_container_width=True):
    ...
    if not filtered.empty:
        ...
        top = top.copy()
        top['推奨比率(%)'] = weights
        st.dataframe(top[['コード', '銘柄名', 'NCR', 'PER', '推奨比率(%)']], 
        ...
        # ベスト版の修正文
        st.success("このポートフォリオは**ネットキャッシュが厚く**、倒産リスクが低い銘柄を中心に構成されています。")
...
st.info("このアプリはYahoo Financeのデータを基にした実用版です。実際の投資判断はご自身でも十分に調査してください。")
st.caption("🛡️ Buffett Pro v6.1 | 実用最終版 | 世界一有能な株式分析官")
